...
New `liquidity/1.2` snapshots display the reported total and reconciled components with exact evidence. Mixed totals remain insufficient-evidence classifications, with no market value; missing components are not silently omitted from an apparently complete breakdown. Existing saved reports remain unchanged until the user explicitly refreshes. Run `tests/test_review_components.py` and staging rollback `tests/database-components.sql` alongside the existing privacy/holdings suites. At this milestone, reviewed internal staging coverage reached eight parent positions, including three mixed totals with eight components; later additions are tracked below and in the development ledger.

### Conditional contractual dates

Migration `20260925191241_reviewed_lockup_timeline.sql` adds reviewed date-only restriction terms with source-scoped, read-only RLS and generated calendar-day boundaries. `scripts/build_lockup_review.py` requires a reproducible existing holdings manifest, an unambiguous final-prospectus cover date, explicit duration, executed executive/director scope, full exceptions and discretionary-release context. It fails closed on unsupported triggers, trading/business-day or graduated schedules. Private manifests and raw passages stay outside Git/frontend assets.

`liquidity/1.3` snapshots include the terms, four evidence passages and deterministic calculation method. A **scheduled boundary is not a confirmed expiry, release, first tradable date or cash-realization date**. No business-day, holiday or intraday convention is invented. The first two timelines are Orion's September 17, 2026 prospectus date + 180 calendar days = March 16, 2027; the filing date and holdings date remain separate. Current liquidity stays unknown, with no quote valuation. Old reports are immutable; only an explicit user refresh captures new terms. Validate with `tests/test_review_lockup.py` and staging rollback `tests/database-lockup.sql`.
...
### Option-only pre-offering disclosures

The `dated-pre-options` reviewer profile accepts a narrow final-prospectus footnote that explicitly distinguishes an option-only pre-offering total from a separate LLC distribution in the post-offering column. It verifies the full named row, date, table assumptions, option quantity and exact pre/post reconciliation, including any disclosed repurchase condition. It imports only the pre-offering option-underlying interests. It does not add the post distribution, equate options to issued common shares, infer personal economic ownership or compute an exercise/intrinsic/cash value.

Migration `20260926002217_reconciled_single_option_component.sql` permits one complete option component to reconcile exactly to its parent beneficial total. Ordinary-share singletons and incomplete/hidden/wrong-document evidence remain rejected. Existing RLS, authorizations and saved reports are unchanged. New staging reports can use the already-deployed component UI; older saved reports require an explicit refresh to capture new evidence. Run `tests/test_option_review.py` and staging rollback `tests/database-options.sql`; generated source packets, manifests and browser fixtures must remain private and ignored.
