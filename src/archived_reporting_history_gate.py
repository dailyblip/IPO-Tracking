...
A prior Exchange Act reporting form must be filed strictly before the candidate.
Same-day reports do not establish event order. Archive lookup failures block final
424B4 publication; for pre-pricing S-1/S-1A rows they do not invent an exclusion,
matching the existing pre-pricing gate's conservative failure behavior.
"""
