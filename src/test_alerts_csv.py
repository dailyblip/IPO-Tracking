...
class AlertsCsvTests(unittest.TestCase):
    def test_export_alerts_csv(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(rows[0]["type"], "price_range_update")
            self.assertEqual(rows[0]["new_value"], "$18-$20")
            self.assertEqual(list(rows[0]), COLUMNS)

    def test_export_empty_alerts_still_writes_header(self):
        with tempfile.TemporaryDirectory() as tmp:
            alerts_path = Path(tmp) / "alerts.json"
            output_path = Path(tmp) / "alerts.csv"
            ...
            self.assertEqual(export_alerts_csv(alerts_path, output_path), 0)
            self.assertEqual(
