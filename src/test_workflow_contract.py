...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_ownership_refresh_defers_live_golden_until_after_regeneration(self):
        workflow = _ownership_workflow()
        unit_test_step = workflow.index("- name: Run unit tests")
        refresh_step = workflow.index("- name: Refresh qualifying IPO history and Stanford affiliations")
